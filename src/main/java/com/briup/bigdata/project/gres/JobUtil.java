...
/**
 * @program: grms.pre
 * @package: com.briup.bigdata.bd1903.project.grms.utils
...
public class JobUtil{
    ...
    public static void setMapper(
        // 设置作业中运行的Mapper类的镜像参数
        Class<? extends Mapper> x,
        // 设置作业中Mapper的Key的数据类型参数
        Class<? extends Writable> y,
        // 设置作业中Mapper的Value的数据类型参数
        Class<? extends Writable> z,
        // 设置作业中数据输入的格式参数
        Class<? extends InputFormat> o
    ){
        try{
            ...
            o.getMethod("addInputPath",Job.class,Path.class)
             .invoke(null,job,new Path(in));
        }catch(Exception e){
            e.printStackTrace();
        }
    }
    ...
    public static void setReducer(
        // 设置作业中运行的Reducer类的镜像参数
        Class<? extends Reducer> a,
        // 设置作业中Reducer的Key的数据类型参数
        Class<? extends Writable> b,
        // 设置作业中Reducer的Value的数据类型参数
        Class<? extends Writable> c,
        // 设置作业中数据输出的格式参数
        Class<? extends OutputFormat> d,
        // 设置Reducer的个数
        int rnum
    ){
        try{
            ...
            job.setNumReduceTasks(Math.max(rnum,1));
        }catch(Exception e){
            e.printStackTrace();
        }
    }
    ...
    public static void setSecondarySort(Class<? extends WritableComparator> g,Class<? extends WritableComparator> s,Class<? extends Partitioner<? extends Writable,? extends Writable>> p
